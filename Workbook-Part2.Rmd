...
library(dplyr)
library(tmap)
tmap_mode("plot") ## Interactive Mapping
```
...
huff_input <- huff_input %>%
  dplyr::mutate(beta = dplyr::case_when(hierarchy == "primary" ~ 1.6,
                                          hierarchy == "secondary" ~ 1.8,
...
In this first part, we show how this can be done for one Retail Centre - St Helens. We extract the LSOAs for the St Helens Retail Centre easily using the filter function, and then map the extent of the catchment identified by the Huff model.

...
We also have two additional parameters (alpha, beta). The default values for these are 1 & 2 respectively, but we have also set beta values as above that differ depending on the hierarchy. Firstly, let's implement a huff model with the default parameters:
...
Remember in section 2b we specified some beta values for different types of Retail Centres:

* 1.8 for Primary centres
* 1.9 for Secondary centres
* 2.0 for Tertiary centres

So this time instead of using the default values (alpha = 1, beta = 2), we will use the beta values we assigned earlier - this involves changing the beta argument below:

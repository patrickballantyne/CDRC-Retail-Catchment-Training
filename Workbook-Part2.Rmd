...
huff_input <- rc %>%
  ...
  as.data.frame() %>%
  select(rcID, rcName,n.comp.units, hierarchy, lsoa11cd, distance)
```
...
* n.comp.units - a count of the number of comparison retail units in each Retail Centre (we saw this in Part 1)
...
It's all there, so we are ready to fit the model. I have included the arguments (e.g. destinations_name) and the columns we are using for each (e.g. rcName) to show you what columns you need to put in which part of the model. But just to remind you:

* A list of unique names for the destination locations - Retail Centre Names (rcName)
* A list of attractiveness score for the destination locations - Attractiveness Indicator for Retail Centres (n.comp.units)
...
We also have two additional parameters (alpha, beta). The default values for these are 1 & 2. Firstly, let's implement a huff model with the default parameters:
...
However, we also want to remove any LSOAs with a huff probability less than 50%, as this will lead to some unusual results. You can do this below with the filter() function:
...
huff_input <- huff_input %>%
  dplyr::mutate(beta = dplyr::case_when(hierarchy == "primary" ~ 2.8,
                                          hierarchy == "secondary" ~ 3.0,
                                            hierarchy == "tertiary" ~ 3.2))
```
...
# huff_input <- mutate(huff_input, beta = case_when(hierarchy == "primary" ~ 2.8,
#                                           hierarchy == "secondary" ~ 3.0,
#                                             hierarchy == "tertiary" ~ 3.2) )
```
...
Now that we have specified the beta values we want to use, we can run thehuff model - still using the default alpha parameter - but this time with the beta values that reflect the hierarchical position of the retail centre. 
...
And the same for the Huff model accounting for hierarchy (Piped and Non-Piped approaches):
...
NOTE: As we are selecting huff probabilities over 50% or more, some LSOAs will be missing. 
...
And now for the Huff model using different beta values for the hierarchies:
...
Notice how the large primary centres (Liverpool City and Southport) have really large catchments. We can increase the probability threshold to 75%  and then map the allocation of LSOAs to see what difference that makes:

```{r}
## Filter rows with huff probability over 75%
lcr_huff_75 <- filter(lcr_hierarchy_huff, huff_probability >= 0.75)

## Map the output
tm_shape(lcr_huff_75) +
  tm_fill(col = "rcName", alpha = 0.4) +
  tm_borders(col = "black", lwd = 0.25, alpha = 0.2) +
  tm_layout(legend.outside = TRUE, legend.outside.position = "right")
```

These catchments look much more sensible! 

## 5. Extracting Huff Catchments 

The final part of this practical is using the huff probabilities and allocation of LSOAs to the Retail Centres to generate Huff catchments for each Retail Centre. We are now just going to work with the results of the Huff model that used beta values to account for hierarchy (lcr_hierarchy_huff), but feel free to run through this next section with the default huff model to see the differences.

In this first part, we show how this can be done for one Retail Centre - Widnes. We extract the LSOAs for the Widnes Retail Centre easily using the filter function, and then map the extent of the catchment identified by the Huff model.

```{r}
## Extract LSOAs and catchment for the Widnes Retail Centre
widnes <- filter(lcr_huff_75, rcName == "Widnes")
```

And extract the corresponding Retail Centre (centroid) too:

```{r}
## Extract the Widnes Retail Centre Centroid
...
```{r}
## Map the Widnes Retail Centre and Huff Catchment
tm_shape(widnes) +
...
That's it! Ok so that's Part 2 of this practical completed, by now you should have a good understanding of:
  ...
  * How to use the results of the Huff model to delineate cathcments for centres.

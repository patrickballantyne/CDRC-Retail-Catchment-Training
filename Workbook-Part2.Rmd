...
However, we also want to remove any LSOAs with a huff probability less than 60%, as otherwise there will be some unusual results, such as LSOAs in South Wirral assigned to the Liverpool City Retail Centre. You can do this below with the filter() function:

```{r}
## Remove LSOAs with huff probs < 50%
top_probs <- filter(top_probs, huff_probability >= 0.6)
```
...
hierarchy_top_probs <- filter(hierarchy_top_probs, huff_probability >= 0.6)
```
...
NOTE: As we are selecting huff probabilities over 50% or more, some LSOAs will be missing from the map; these are the LSOAs containing huff probabilities less than 50%.
...
huyton <- filter(lcr_huff, rcName == "Huyton")
```
...
This practical was conceptualised and written using R and RStudio by Patrick Ballantyne (P.J.Ballantyne@liverpool.ac.uk) and Les Dolega (L.Dolega@liverpool.ac.uk).
